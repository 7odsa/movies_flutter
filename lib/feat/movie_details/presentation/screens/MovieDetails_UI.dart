...
import 'package:movies_flutter/feat/movie_details/presentation/widgets/favourites_button.dart'show FavoriteButton;
          ...
          builder: (context, state) {
            ...
            Column(
              children: [
                const Center(
                  heightFactor: 20,
                child: Icon(Icons.play_circle_fill, size: 64, color: Colors.yellow),
                ),
              ],
            ),
            ],
            ...
            const SizedBox(height: 12),
                      ...
                      ElevatedButton(
                        onPressed: () async {
                          final url = Uri.parse('https://www.youtube.com/watch?v=${movie.trailerCode}');
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          _infoIcon(Icons.confirmation_number, 'IMDB: ${movie.imdbCode}'),
                        ],
                      ),

                      const SizedBox(height: 20),

                      const Text('Screen Shots', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 12),
                      Column(
                      ...
                      Wrap(
                        spacing: 8,
                        runSpacing: 8,
                        children: state.similarMovies.map((similarMovie) {
                          return GestureDetector(
                            onTap: () {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (_) => MovieDetailsScreen(movieId: similarMovie.id),
                                ),
                              );
                            },
                            child: SizedBox(
...
class CastItem extends StatelessWidget {
  ...
  const CastItem({super.key, required this.name, required this.character, this.imageUrl});
